...
class Reader_jrdb_2dbox(object):
    ...
    def paths_to_ids(paths):
        """Convert paths to ids (frame_id, pedestrian_id)."""
        frames = set(r.frame for r in paths[0])
        pedestrians = set(row.pedestrian
                          for path in paths for row in path if row.frame in frames)
        paths = [path for path in paths if path[0].pedestrian in pedestrians]
        frames = sorted(frames)
        pedestrians = list(pedestrians)

        frame_to_index = {frame: i for i, frame in enumerate(frames)}
        ids = np.full((len(frames), len(pedestrians), 2), np.nan)

        for ped_index, path in enumerate(paths):
            for row in path:
                if row.frame not in frame_to_index:
                    continue
                ids[frame_to_index[row.frame]][ped_index] = row.frame, row.pedestrian
        return ids
    ...
    def paths_to_xy(paths):
        ...
        for ped_index, path in enumerate(paths):
            for row in path:
                ...
                ## extract trajectory
                entry[0] = row[2] # position x
                entry[1] = row[3] # position y
                entry[2] = 0.0 # position z -> 0.0
                entry[3] = 0.0 # leave the last one as 0.0

                for b in range(8):
                    entry[b+4] = row[b+4] # info for 2d and 3d BB


                for j in range(22):
                    entry[j*4+12] = row[j+12] # x of 3d joints
                    entry[j*4+13] = row[j+12+22] # y of 3d joints
                    entry[j*4+14] = row[j+12+22+22] # z of 3d joints
                    entry[j*4+15] = 0.0 # leave the last one as 0.0

                    entry[j*4+12+88] = row[j+78] # x of 2d joints
                    entry[j*4+13+88] = row[j+78+22] # y of 2d joints
                    entry[j*4+14+88] = 0.0 # z of 2d joints -> 0.0
                    entry[j*4+15+88] = 0.0 # leave the last one as 0.0
