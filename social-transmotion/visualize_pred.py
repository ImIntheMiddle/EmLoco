...
class CompareVis_3D(Visualizer_3D):
    def save_plot(
        ...
        # save plot
        plt.savefig(
            os.path.join(self.save_dir, f"batch{id_b}_person{id_k}.png"),
            bbox_inches="tight",
        )
...
if __name__ == "__main__":
    ...
    # `paths` is a per-user mapping of <display-name> -> (exp_name, modality)
    # tuples picking which experiments / modalities to overlay in the figure.
    # Fill it in before running, e.g.:
    #     paths = {
    #         "Ours":     ("jta_ours",        "traj+all"),
    #         "Baseline": ("jta_st_standard", "traj+all"),
    #     }
    paths = {}
    if not paths:
        raise SystemExit(
            "visualize_pred.py: please populate the `paths` dict in this file "
            "(near the top of __main__) with the experiments you want to "
    ...
    for frame_num in [args.frame_num]:
        ...
        if not args.use_vis_dict:
            for i in range(1, len(data)):
                assert len(data[i][1]) == len(data[i - 1][1]), (
                    f"Data length mismatch: {len(data[i][1])} != {len(data[i - 1][1])}"
                )
