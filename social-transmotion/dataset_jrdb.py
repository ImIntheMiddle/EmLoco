...
class MultiPersonTrajPoseDataset(torch.utils.data.Dataset):
    ...
    def show_meta_info(self, idx):
        ...
        people = self.datalist[idx]
        if people and len(people[0]) > 2:
            return [s[2] for s in people]
        # Older 2-tuple shards carry no per-person meta.
        return [("unknown", None)] * len(people)
