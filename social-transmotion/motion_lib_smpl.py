...
import sys
sys.path.append("/misc/dl00/halo/plausibl/")
...
from pacer.pacer.utils import torch_utils
import joblib
import torch
...
import copy
import gc
from smpllib.smpl_parser import (
    SMPL_Parser,
    SMPLH_Parser,
    SMPLX_Parser,
)
...
class MotionLib():
    def __init__(self, motion_file, key_body_ids, device, fix_height = True, masterfoot_conifg = None, min_length = -1):
        ...
        motion_list = list(len(self._motion_data_list[i]['pose_aa']) for i in range(len(self._motion_data_list)))
        # import pdb; pdb.set_trace()

        self._masterfoot_conifg = masterfoot_conifg
        data_dir = "../pacer/data/smpl"
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample = True, start_idx = 0):
        ...
        total_len = 0.0
        skeleton_trees = [skeleton_trees] * self._num_unique_motions
        gender_betas = torch.Tensor(np.array([gender_betas] * self._num_unique_motions))
        limb_weights = torch.Tensor(np.array([limb_weights] * self._num_unique_motions))

        num_motion_to_load = 2 if random_sample else len(skeleton_trees)
        ...
        print("Sampling motion in total:", len(sample_idxes))
        ...
        print("Finished loading motions")
        ...
        # self._motion_limb_weights = torch.tensor(limb_weights,
                                                # device=self._device,
                                                # dtype=torch.float32)
        self._motion_limb_weights = limb_weights.to(self._device).type(torch.float32)
    ...
    def save_global_translation(self, savedir, filename):
        if not os.path.exists(savedir):
            os.makedirs(savedir)
        save_dict = {}
        save_dict['gts'] = self.gts
        save_dict['start'] = self.length_starts
        # import pdb; pdb.set_trace()
        with open(os.path.join(savedir, filename), 'wb') as f:
            pickle.dump(save_dict, f)
        print("Saved global translation to", os.path.join(savedir, filename))
        
    def get_global_translation(self):
        return self.gts, self.length_starts
