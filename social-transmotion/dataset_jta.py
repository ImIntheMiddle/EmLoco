...
def batch_process_coords(
    ...
    joints[:, :, :, 1:3] = (
        joints[:, :, :, 1:3] - joints[:, :, (in_F - 1) : in_F, 1:3]
    )  # normalize other joints
    joints[:, :, :, 3:27] = (
        joints[:, :, :, 3:27] - joints[:, :, (in_F - 1) : in_F, 3:27]
    )  # normalize 3d poses #nonnorm or not
    joints[:, :, :, 27:] = (
        joints[:, :, :, 27:] - joints[:, :, (in_F - 1) : in_F, 27:]
    )  # normalize 2d poses
...
class MultiPersonTrajPoseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if preprocessed:
            self.datalist = []
            # CVPR 2025 EmLoco JTA shards: J=49 token layout
            # (1 traj + 4 3dbb + 4 2dbb + 20 3dpose + 20 2dpose), torch 2.x zip-format .pt.
            saved_dir = f"data/{self.name}/preprocess_smpl_cvpr/{self.split}"
            load_bar = tqdm.tqdm(os.listdir(saved_dir))
...
class JtaAllVisualCuesDataset(MultiPersonTrajPoseDataset):
    def __init__(self, **args):
        super(JtaAllVisualCuesDataset, self).__init__(
            "jta_all_visual_cues", frequency=1, **args
        )
