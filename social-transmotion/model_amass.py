...
class TransMotionAMASS(TransMotionJTA):
    def __init__(self, tok_dim=21, nhid=256, nhead=4, dim_feedfwd=1024, nlayers_local=2, nlayers_global=4, nmode=5, dropout=0.1, activation='relu', output_scale=1, obs_and_pred=21,  num_tokens=47, device='cuda:0', multi_modal=False):
        super(TransMotionAMASS, self).__init__(tok_dim, nhid, nhead, dim_feedfwd, nlayers_local, nlayers_global, nmode, dropout, activation, output_scale, obs_and_pred, num_tokens, device, multi_modal=multi_modal)
    ...
    def forward(self, tgt, padding_mask, random_masking=False, limit_obs=0, frame_masking=False, noisy_traj=False):
        ...
        modality_selection = (torch.rand((B,1,N,1)).float().to(self.device) > mask_ratio_modality).unsqueeze(4).repeat(1,F,1,self.joints_3dpose,4) # modalities: 3dpose only
        ...
        tgt_3dpose = tgt_3dpose[:,:12].transpose(2,3).reshape(B,-1,N,3)
        ...
        out_local = out_local[:F].reshape(F,B,N,self.nhid).permute(2,0,1,3).reshape(-1,B,self.nhid)
...
def create_model(config, logger):
    ...
    if config["MODEL"]["type"] == "transmotion":
        ...
        model = TransMotionAMASS(tok_dim=seq_len,
            nhid=nhid,
            nhead=nhead,
            nmode=nmode,
