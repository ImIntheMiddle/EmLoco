...
def evaluate_loss(
    model, dataloader, valuenet, config, limit_obs=False, modality_selection="traj+all"
):
    in_F, out_F = (
        config["TRAIN"]["input_track_size"],
        config["TRAIN"]["output_track_size"],
    )
    bar = Bar("EVAL", fill="#", max=len(dataloader))
    ...
    with torch.no_grad():
        ...
        for i in range(len(dataloader)):
            ...
            mse_loss, pred_joints = compute_loss(
                model,
                config,
                in_joints,
                out_joints,
            ...
            if valuenet is not None:
                if config["MULTI_MODAL"]:
                    ...
                    pred_trajs = torch.cat(
                        [
                            torch.zeros(
                                pred_trajs.size(0), 1, pred_trajs.size(2), 2
                            ).to(config["DEVICE"]),
                            pred_trajs,
                        ],
                        dim=1,
                    )
                    pred_trajs, primary_init_pose, primary_init_vel = nan_handler(
                    ...
                    for i in range(pred_trajs.size(2)):
                        pred_value, value_loss = valuenet.calc_embodied_motion_loss(
                            pred_trajs[:, :, i], primary_init_pose, primary_init_vel
                        )
                        value_losses += value_loss
                    value_losses *= config["TRAIN"]["valuenet_weight"]
                    value_loss = (
                        value_losses / pred_trajs.size(2)
                        if pred_trajs.size(2) > 0
                        else 0
                    )
                else:
                    ...
                    pred_traj = torch.cat(
                        [
                            torch.zeros(pred_traj.size(0), 1, 2).to(config["DEVICE"]),
                            pred_traj,
                        ],
                        dim=1,
                    )
...
def compute_loss(
    ...
    if config["NOISY_TRAJ"]:  # gaussian(0, 0.5^2) when NOISY_TRAJ=0.5
        in_joints[:, :, 0, :2] = (
            in_joints[:, :, 0, :2]
            + torch.randn_like(in_joints[:, :, 0, :2]) * config["NOISY_TRAJ"] ** 2
        )
        out_joints[:, :, 0, :2] = (
            out_joints[:, :, 0, :2]
            + torch.randn_like(out_joints[:, :, 0, :2]) * config["NOISY_TRAJ"] ** 2
        )
    ...
    pred_joints = model(
        ...
        random_masking,
        limit_obs=limit_obs,
        frame_masking=config["USE_FRAME_MASK"],
    )
...
def prepare(config, logger, experiment_name, dataset_name):
    ...
    if config["USE_VALUELOSS"]:
        valuenet = ValuePoseNet(
            use_pose=config["USE_POSE"], use_vel=config["USE_VELOCITY"]
        )
        assert config["MODEL"]["valuenet_checkpoint"] != "", (
            "Please provide a checkpoint for the valuenet!"
        )
    ...
    dataloader_train = dataloader_for(
        dataset_train, config, shuffle=True, pin_memory=True
    )
    ...
    dataloader_val = dataloader_for_val(
        dataset_val, config, shuffle=True, pin_memory=True
    )
...
def train(
    ...
    bar = Bar(
        f"TRAIN {epoch}/{config['TRAIN']['epochs'] - 1}", fill="#", max=train_steps
    )
    ...
    for i in range(train_steps):
        ...
        if valuenet is not None:
            if config["MULTI_MODAL"]:
                ...
                pred_trajs = torch.cat(
                    [
                        torch.zeros(pred_trajs.size(0), 1, pred_trajs.size(2), 2).to(
                            config["DEVICE"]
                        ),
                        pred_trajs,
                    ],
                ...
                value_losses *= config["TRAIN"]["valuenet_weight"]
                value_loss = (
                    value_losses / pred_trajs.size(2) if pred_trajs.size(2) else 0
                )
            else:
                pred_traj = pred_joints[:, in_F:].squeeze(2)
                pred_traj = torch.cat(
                ...
                value_loss *= config["TRAIN"]["valuenet_weight"]
            # Filter both NaN and Inf — same rationale as the evaluate_loss path.
            finite_mask = torch.isfinite(value_loss)
            finite_value_loss = value_loss[finite_mask] if finite_mask.any() else None
        ...
        ################################
        # Guard against rare numerical explosions in mse_loss / value_loss that survive
        # per-element filtering — backprop on a NaN/Inf scalar corrupts every weight via
        # the gradient and the run never recovers. Skip the batch instead.
        if not torch.isfinite(loss):
            print(
                f"[epoch {epoch} step {i}] non-finite loss detected (mse={mse_loss:.4f}), skipping batch"
            )
        ...
        if config["VAL_LOSS_ONLY"] and i % 200 == 0:
            ...
            if val_loss_tmp / 100 < min_val_loss:
                logger.info(
                    f"Epoch {epoch} | Train Loss: {loss_avg.avg:.3f} | Val ADE: {(val_loss_tmp / 100):.3f}"
                )
                ...
                save_checkpoint(
                    ...
                    epoch,
                    config,
                    "best_val" + "_checkpoint_99epoch.pth.tar",
                    logger,
                )
    ...
    logger.info(
        f"Epoch {epoch} | Train Loss: {loss_avg.avg:.3f} | Val ADE: {val_ade:.3f}"
    )
...
def main(
    ...
    if config["RESUME"] != -1:
        if cfg["MODEL"]["checkpoint"] == "":
            ...
            if os.path.exists(
                os.path.join(cfg["OUTPUT"]["ckpt_dir"], "checkpoint.pth.tar")
            ):
                cfg["MODEL"]["checkpoint"] = os.path.join(
                    cfg["OUTPUT"]["ckpt_dir"], "checkpoint.pth.tar"
                )
            elif os.path.exists(
                os.path.join(cfg["OUTPUT"]["ckpt_dir"], "best_val_checkpoint.pth.tar")
            ):
                cfg["MODEL"]["checkpoint"] = os.path.join(
                    cfg["OUTPUT"]["ckpt_dir"], "best_val_checkpoint.pth.tar"
                )
            else:
    ...
    writer_name = (
        experiment_name + "_" + str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    )
    ...
    if valuenet is not None:
        logger.info(
            f"Using Value Loss weight: {float(config['TRAIN']['valuenet_weight']):.3f}"
        )
    ...
    for epoch in range(config["RESUME"] + 1, config["TRAIN"]["epochs"]):
        val_ade = train(
            ...
            limit_obs=limit_obs,
            modality_selection=config["MODALITY"],
        )
        if val_ade < min_val_loss:
            ...
            save_checkpoint(
                ...
                epoch,
                config,
                f"best_val_checkpoint_{epoch}epoch.pth.tar",
                logger,
            )
        if epoch % 5 == 0:
            save_checkpoint(
                model, optimizer, epoch, config, "checkpoint.pth.tar", logger
            )
        if config["dry_run"]:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Experiment name. Otherwise will use timestamp",
    )
    parser.add_argument(
        "--cfg",
        type=str,
        default="configs/jta_all_visual_cues.yaml",
    ...
    parser.add_argument(
        "--resume", type=int, default=-1, help="Resume training from a checkpoint"
    )
    parser.add_argument(
        "--not_pose",
        action="store_true",
        help="Not using pose input for value function",
    )
    parser.add_argument(
        "--not_vel",
        action="store_true",
        help="Not using velocity input for value function",
    )
    parser.add_argument(
        "--limit_obs", type=int, default=0, help="Limit the number of observations"
    )
    parser.add_argument(
        "--frame_mask", type=bool, default=True, help="Use frame masking"
    )
    parser.add_argument(
        "--value_path",
        type=str,
        default="",
        help="Path to the value network checkpoint. For example, 'valuenet_realpath_JTA+JRDB_valuenet_00025000.pth'. ",
    )
    parser.add_argument(
        "--value_dir",
        type=str,
        default="../pacer/output/exp/pacer/",
        help="Directory to the value network checkpoint",
    )
    parser.add_argument(
        "--noisy_traj",
        type=float,
    ...
    parser.add_argument(
        "--valueloss_only", action="store_true", help="Train with the value loss only"
    )
    parser.add_argument(
        "--modality",
        type=str,
        default="traj+all",
        help="available modality combination from['traj','traj+2dbox','traj+3dpose','traj+2dpose','traj+3dpose+3dbox','traj+all', 'traj+2dbox+2dpose', 'traj+2dbox+2dpose+3dbox', 'traj+2dpose+3dpose']",
    )
    ...
    if args.value_path != "":
        cfg["MODEL"]["valuenet_checkpoint"] = os.path.join(
            args.value_dir, args.value_path
        )
    elif os.sep not in cfg["MODEL"]["valuenet_checkpoint"]:
        # Config holds just a filename — resolve against --value_dir.
        cfg["MODEL"]["valuenet_checkpoint"] = os.path.join(
            args.value_dir, cfg["MODEL"]["valuenet_checkpoint"]
        )
    # else: config already holds a full path; use as-is.
    ...
    valuenet, dataloader_train, dataloader_val = prepare(
        cfg, logger, args.exp_name, dataset
    )

    min_val_loss, best_epoch = main(
        cfg,
        logger,
        valuenet,
        dataloader_train,
