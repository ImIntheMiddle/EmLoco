...
def inference(model, config, input_joints, padding_mask, out_len=14, limit_obs=False):
    ...
    with torch.no_grad():
        if torch.isnan(input_joints).any():
            ...
            input_joints = torch.where(
                torch.isnan(input_joints), torch.zeros_like(input_joints), input_joints
            )
...
class Visualizer_3D:
    ...
    def plot_3d(
        ...
        if past_len == 1:
            ...
            ax.plot(
                past_xy[-past_len:, 0],
                past_xy[-past_len:, 1],
                np.zeros(past_len) - 1.5,
                c="k",
                label=f"Past Trajectory ({past_len}frame)",
                linewidth=1.5,
                linestyle="-",
                marker="o",
                markersize=3,
    ...
    def save_plot(
        ...
        # save
        plt.savefig(
            os.path.join(self.save_dir, f"batch{id_b}_person{id_k}_ade{ade[0]:.2f}.png")
        )
        plt.savefig(
            os.path.join(
                self.save_dir, f"batch{id_b}_person{id_k}_ade{ade[0]:.2f}.pdf"
            ),
            bbox_inches="tight",
        )
        plt.close()
...
def evaluate_ade_fde(
    ...
    if num_mode > 1:
        ...
        plt.bar(
            bin_centers,
            ade_mean_values,
            color=plt.cm.viridis(bin_centers),
            width=0.1,
            alpha=0.9,
            edgecolor="white",
        )
        ...
        for i in range(len(bin_centers)):
            if ade_mean_values[i] > 0:
                ...
                plt.text(
                    bin_centers[i],
                    ade_mean_values[i],
                    f"{ade_mean_values[i]:.2f}",
                    ha="center",
                    va="bottom",
                    fontsize=11,
                )
        plt.savefig(
            os.path.join("./experiments/JTA", exp_name, "value_ade_barplot.png")
        )
        # pdf
        plt.savefig(
            os.path.join("./experiments/JTA", exp_name, "value_ade_barplot.pdf"),
            bbox_inches="tight",
        )
        plt.close()
        plt.bar(
        ...
        plt.savefig(
            os.path.join("./experiments/JTA", exp_name, "value_fde_barplot.png")
        )
        plt.savefig(
            os.path.join("./experiments/JTA", exp_name, "value_fde_barplot.pdf"),
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.savefig(
            os.path.join("./experiments/JTA", exp_name, "value_hist.pdf"),
            bbox_inches="tight",
        )
    ...
    if visualize:
        ...
        with open(
            os.path.join(save_dir, f"vis_dict_{past_frame_num}frame.pkl"), "wb"
        ) as f:
            pickle.dump(vis_dict, f)
        logger.info(
            f"vis_dict saved to {os.path.join(save_dir, f'vis_dict_{past_frame_num}frame.pkl')}!"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="traj+all",
        help="available modality combination from['traj','traj+2dbox','traj+3dpose','traj+2dpose','traj+3dpose+3dbox','traj+all', 'traj+2dbox+2dpose', 'traj+2dbox+2dpose+3dbox]",
    )
    ...
    parser.add_argument(
        "--all_frames", action="store_true", help="Evaluate all observation frames"
    )
    parser.add_argument(
        "--noisy_traj",
        type=float,
    ...
    parser.add_argument(
        "--last_epoch", action="store_true", help="Use last epoch checkpoint"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Threshold for filtering samples",
    )
    ...
    if args.last_epoch:
        ...
        if os.path.exists(
            f"./experiments/JTA/{args.exp_name}/checkpoints/best_val_checkpoint_{args.epoch}epoch.pth.tar"
        ):
            ckpt_name = f"./experiments/JTA/{args.exp_name}/checkpoints/best_val_checkpoint_{args.epoch}epoch.pth.tar"
        elif os.path.exists(
            f"./experiments/JTA/{args.exp_name}/checkpoints/checkpoint_{args.epoch}epoch.pth.tar"
        ):
            ...
            f"./experiments/JTA/{args.exp_name}/checkpoints/best_val_{args.epoch}.pth.tar"
        ):
    ...
    # Allow checkpoints trained with a modality subset (e.g. traj+3dpose only) to load by
    # tolerating missing modality-specific layers. With the matching --modality flag those
    # layers are zeroed out during inference, so the load is harmless.
