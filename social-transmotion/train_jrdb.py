    ...
    for i in range(train_steps):
        ...
        if valuenet is not None:
            if config["MULTI_MODAL"]:
                ...
                value_losses *= config["TRAIN"]["valuenet_weight"]
                value_loss = (
                    value_losses / pred_trajs.size(2) if pred_trajs.size(2) > 0 else 0
                )
            else:
                pred_traj = pred_joints[:, in_F:].squeeze(2)
                pred_traj = torch.cat(
...
def main(
    ...
    if cfg["RESUME"] != -1:
        if cfg["MODEL"]["checkpoint"] == "":
            ...
            if os.path.exists(
                os.path.join(
                    cfg["OUTPUT"]["ckpt_dir"], f"checkpoint_{last_epoch}epoch.pth.tar"
                )
            ):
    ...
    writer_train = SummaryWriter(
        os.path.join(cfg["OUTPUT"]["runs_dir"], f"{writer_name}_TRAIN")
    )
    writer_valid = SummaryWriter(
        os.path.join(cfg["OUTPUT"]["runs_dir"], f"{writer_name}_VALID")
    )
    ...
    min_val_loss = (
        1e6
        if not cfg["RESUME"]
        else evaluate_loss(model, dataloader_val, valuenet, cfg) / 100
    )
    best_epoch = cfg["RESUME"] if cfg["RESUME"] else 0
    logger.info(f"Initial validation loss: {min_val_loss:.3f}")
    if valuenet is not None:
        logger.info(
            f"Using Value Loss weight: {float(cfg['TRAIN']['valuenet_weight']):.3f}"
        )
    ...
    for epoch in range(cfg["RESUME"] + 1, cfg["TRAIN"]["epochs"]):
        val_ade = train(
            ...
            limit_obs=False,
            modality_selection=cfg["MODALITY"],
        )
        if val_ade < min_val_loss:
            ...
            best_epoch = epoch
            save_checkpoint(
                model, optimizer, epoch, cfg, "best_val" + "_checkpoint.pth.tar", logger
            )
            save_checkpoint(
                model,
                optimizer,
                epoch,
                cfg,
                "best_val" + f"_checkpoint_{epoch}epoch.pth.tar",
                logger,
            )
        if epoch % 5 == 0:
            save_checkpoint(
                model, optimizer, epoch, cfg, f"checkpoint_{epoch}epoch.pth.tar", logger
            )
        if cfg["dry_run"]:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg",
        type=str,
        default="configs/jrdb_all_visual_cues.yaml",
    ...
    parser.add_argument(
        "--valueloss_w", type=float, default=0, help="Weight for value loss"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limit_obs", type=int, default=0, help="Limit the number of past observations"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="traj+all",
        help="available modality combination from['traj','traj+all', 'traj+2dbox','traj+3dpose']",
    )
