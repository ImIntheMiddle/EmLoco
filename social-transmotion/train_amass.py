...
from dataset_amass import collate_batch, batch_process_coords, get_datasets, create_dataset
...
def evaluate_loss(model, dataloader, valuenet, config, limit_obs=False, modality_selection='traj+all'):
    ...
    with torch.no_grad():
        for i in range(len(dataloader)):
            ...
            primary_init_pose = in_joints[:,11,1:25,:3].to(config["DEVICE"])

            primary_init_vel = (in_joints[:,11,0,:2] - in_joints[:,10,0,:2]) * 30 # 30 fps
...
def prepare(config, logger, experiment_name, dataset_name):
    ...
    if config["USE_VALUELOSS"]:
        valuenet = ValuePoseNet(use_pose=config["USE_POSE"], use_vel=config["USE_VELOCITY"], amass=True)
...
def train(config, epoch, model, optimizer, logger, valuenet, dataloader_train, dataloader_val, min_val_loss, experiment_name="", dataset_name="", limit_obs=False, modality_selection='traj+all'):
    ...
    for i in range(train_steps):
        ...
        padding_mask = padding_mask.to(config["DEVICE"])
        primary_init_pose = in_joints[:, 11, 1:, :3].clone().to(config["DEVICE"])
...
def main(cfg, logger, valuenet, dataloader_train, dataloader_val, experiment_name, dataset_name, limit_obs):
    ...
    model = create_model(cfg, logger).to(cfg["DEVICE"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", type=str, default="configs/amass_all_visual_cues.yaml", help="Config name. Otherwise will use default config")
    ...
    if args.cfg != "":
        cfg = load_config(args.cfg, exp_name=args.exp_name, dataset_name="AMASS")
    else:
