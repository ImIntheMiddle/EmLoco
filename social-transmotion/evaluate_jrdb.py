...
from utils.utils import create_logger, load_config
from utils.metrics import (
    calculate_initial_yaw_error,
    calculate_velocity,
    calculate_acceleration,
    calculate_ang_velocity,
    calculate_ang_acceleration,
    calculate_chi_distance,
)
...
def evaluate_ade_fde(
    ...
    if visualize:
        ...
        if valuenet is not None:
            ...
            save_dir = os.path.join(
                save_dir, valuenet_path.split("/")[-1].split(".")[0]
            )
    ...
    for i, batch in enumerate(bar):
        ...
        pred_joints = inference(
            model, config, in_joints, padding_mask, out_len=out_F, limit_obs=limit_obs
        )
        ...
        iye_batch += calculate_initial_yaw_error(
            out_joints[:, 0, 0, :2], pred_joints[:, 0, 0, :2]
        ).sum()  # all scene, initial frame, primary person
        ...
        for k in range(len(out_joints)):
            ...
            init_pose = (
                primary_init_pose[k].clone().detach().to(config["DEVICE"]).float()
            )
            init_pose[..., 0] = -init_pose[..., 0]  # transformation for jrdb dataset
            init_vel = (
                in_joints[k, 8, 0, :2] - in_joints[k, 7, 0, :2]
            ).clone().detach() * 2.5
            ...
            gt_primitive["velocity"] = (
                np.concatenate((gt_primitive["velocity"], gt_vel), axis=0)
                if len(gt_primitive["velocity"])
                else gt_vel
            )
            gt_accel = calculate_acceleration(gt_vel)
            gt_primitive["acceleration"] = (
                np.concatenate((gt_primitive["acceleration"], gt_accel), axis=0)
                if len(gt_primitive["acceleration"])
                else gt_accel
            )
            gt_angvel = calculate_ang_velocity(gt_xy)
            gt_primitive["ang_velocity"] = (
                np.concatenate((gt_primitive["ang_velocity"], gt_angvel), axis=0)
                if len(gt_primitive["ang_velocity"])
                else gt_angvel
            )
            gt_angaccel = calculate_ang_acceleration(gt_angvel)
            gt_primitive["ang_acceleration"] = (
                np.concatenate((gt_primitive["ang_acceleration"], gt_angaccel), axis=0)
                if len(gt_primitive["ang_acceleration"])
                else gt_angaccel
            )
            ...
            for p in range(pred_xys.size(1)):
                ...
                pred_primitive["velocity"] = (
                    np.concatenate((pred_primitive["velocity"], pred_vel), axis=0)
                    if len(pred_primitive["velocity"])
                    else pred_vel
                )
                pred_accel = calculate_acceleration(pred_vel)
                pred_primitive["acceleration"] = (
                    np.concatenate((pred_primitive["acceleration"], pred_accel), axis=0)
                    if len(pred_primitive["acceleration"])
                    else pred_accel
                )
                pred_angvel = calculate_ang_velocity(pred_xy)
                pred_primitive["ang_velocity"] = (
                    np.concatenate(
                        (pred_primitive["ang_velocity"], pred_angvel), axis=0
                    )
                    if len(pred_primitive["ang_velocity"])
                    else pred_angvel
                )
                pred_angaccel = calculate_ang_acceleration(pred_angvel)
                pred_primitive["ang_acceleration"] = (
                    np.concatenate(
                        (pred_primitive["ang_acceleration"], pred_angaccel), axis=0
                    )
                    if len(pred_primitive["ang_acceleration"])
                    else pred_angaccel
                )
                ...
                gt_xy = (
                    np.concatenate((np.zeros((1, 2)), gt_xy), axis=0)
                    if len(gt_xy) == out_F
                    else gt_xy
                )
                gt_traj = (
                    torch.tensor(gt_xy).to(config["DEVICE"]).float()
                    if type(gt_xy) != torch.Tensor
                    else gt_xy
                )
                pred_xy = np.concatenate((np.zeros((1, 2)), pred_xy), axis=0)
                pred_traj = torch.tensor(pred_xy).to(config["DEVICE"]).float()
                if not torch.isnan(init_pose).any():
                    sample_with_pose += 1
                if (
                    ...
                    with torch.no_grad():
                        ...
                        pred_value_gt, value_loss_gt = (
                            valuenet.calc_embodied_motion_loss(
                                gt_traj.unsqueeze(0),
                                init_pose.unsqueeze(0),
                                init_vel.unsqueeze(0),
                            )
            ...
            if num_mode > 1 and valuenet is not None:
                ...
                if len(pred_values) > 0:
                    ...
                    filtered_candidates = [
                        c
                        for c in candidates
                        if pred_values[candidates.index(c)] >= filter_threshold
                    ]
                    out_candidates = [
                        c
                        for c in candidates
    ...
    for action in action_list:
        ...
        fde[action] = (
            fde_batch[action] / sample_num[action] if sample_num[action] != 0 else 0
        )
        min_ade[action] = (
            ade_batch_min[action] / sample_num[action] if sample_num[action] != 0 else 0
        )
        min_fde[action] = (
            fde_batch_min[action] / sample_num[action] if sample_num[action] != 0 else 0
        )
        des[action] = (
            des_batch[action] / sample_num[action] if sample_num[action] != 0 else 0
        )
        ...
        logger.info(
            f"{action} ADE: {ade[action]:.5f}, FDE: {fde[action]:.5f}, minADE: {min_ade[action]:.5f}, minFDE: {min_fde[action]:.5f}, sample_num: {sample_num[action]}"
        )
    ...
    iye = iye_batch / (
        (batch_id - 1) * batch_size + len(out_joints)
    )  # initial yaw error
    ...
    if num_mode > 1:
        ade_value_sample = (
            ade_value / sample_num_value_sampling
            if sample_num_value_sampling > 0
            else 0
        )
        fde_value_sample = (
            fde_value / sample_num_value_sampling
            if sample_num_value_sampling > 0
            else 0
        )
        ade_random_sample = (
            ade_random / sample_num_random if sample_num_random > 0 else 0
        )
        fde_random_sample = (
            fde_random / sample_num_random if sample_num_random > 0 else 0
        )
        ...
        ade_filtered = (
            ade_filtered / sample_num_filtered if sample_num_filtered > 0 else 0
        )
        fde_filtered = (
            fde_filtered / sample_num_filtered if sample_num_filtered > 0 else 0
        )
        ...
        fde_mean_values = [
            fde_array[value_indices == i].mean()
            if np.any(value_indices == i)
            else np.nan
            for i in range(1, len(bins))
        ]
        ...
        for i in range(len(bin_centers)):
            if ade_mean_values[i] > 0:
                plt.text(
                    bin_centers[i],
                    ade_mean_values[i],
                    f"{len(ade_array[value_indices == (i + 1)])}",
                    ha="center",
                    va="bottom",
                )
        plt.savefig(
            os.path.join("./experiments/JRDB", exp_name, "value_ade_barplot.png")
        )
        plt.close()
        ...
        for i in range(len(bin_centers)):
            if fde_mean_values[i] > 0:
                plt.text(
                    bin_centers[i],
                    fde_mean_values[i],
                    f"{len(fde_array[value_indices == (i + 1)])}",
                    ha="center",
                    va="bottom",
                )
        plt.savefig(
            os.path.join("./experiments/JRDB", exp_name, "value_fde_barplot.png")
        )
        plt.close()
...
def get_action_label(split, metainfo, action_dict):
    # Older 2-tuple shards (jrdb_3dpose/preprocess_smpl) have no per-person meta;
    # the dataset substitutes ("unknown", None) — skip action lookup gracefully.
    if metainfo[0] == "unknown":
        return "None"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="traj+all",
        help="available modality combination from['traj','traj+2dbox','traj+3dpose','traj+all']",
    )
    parser.add_argument("--vis", action="store_true", help="Visualize the predictions")
    parser.add_argument(
        "--limit_obs", type=int, default=0, help="Limit the number of observations"
    )
    ...
    parser.add_argument(
        "--multi_modal", action="store_true", help="Use multi-modal model"
    )
    parser.add_argument(
        "--all_frames", action="store_true", help="Use all frames for evaluation"
    )
    parser.add_argument(
        "--last_epoch",
        action="store_true",
        help="Load the last checkpoint (checkpoint.pth.tar) instead of best_val_checkpoint.pth.tar",
    )
    ...
    if args.last_epoch:
        ...
        if not os.path.exists(ckpt_name):
            raise FileNotFoundError(
                f"--last_epoch requested but {ckpt_name} does not exist"
            )
    elif args.epoch != 0:
        if os.path.exists(
            f"./experiments/JRDB/{args.exp_name}/checkpoints/best_val_checkpoint_{args.epoch}epoch.pth.tar"
        ):
            ckpt_name = f"./experiments/JRDB/{args.exp_name}/checkpoints/best_val_checkpoint_{args.epoch}epoch.pth.tar"
        elif os.path.exists(
            f"./experiments/JRDB/{args.exp_name}/checkpoints/checkpoint_{args.epoch}epoch.pth.tar"
        ):
            ckpt_name = f"./experiments/JRDB/{args.exp_name}/checkpoints/checkpoint_{args.epoch}epoch.pth.tar"
        elif os.path.exists(
            f"./experiments/default/{args.exp_name}/checkpoints/best_val_{args.epoch}.pth.tar"
        ):
            ckpt_name = f"./experiments/default/{args.exp_name}/checkpoints/best_val_{args.epoch}.pth.tar"
        elif os.path.exists(
            f"./experiments/JRDB/{args.exp_name}/checkpoints/best_val_{args.epoch}.pth.tar"
        ):
            ckpt_name = f"./experiments/JRDB/{args.exp_name}/checkpoints/best_val_{args.epoch}.pth.tar"
        else:
            raise FileNotFoundError("Checkpoint not found")
    elif os.path.exists(
        f"./experiments/JRDB/{args.exp_name}/checkpoints/best_val_checkpoint.pth.tar"
    ):
        ckpt_name = f"./experiments/JRDB/{args.exp_name}/checkpoints/best_val_checkpoint.pth.tar"
    elif os.path.exists(
        f"./experiments/JRDB/{args.exp_name}/checkpoints/checkpoint.pth.tar"
    ):
        ckpt_name = f"./experiments/JRDB/{args.exp_name}/checkpoints/checkpoint.pth.tar"
    else:
        raise FileNotFoundError("Checkpoint not found")
    ...
    if args.valueloss:
        ...
        valuenet_ckpt = new_cfg["MODEL"].get(
            "valuenet_checkpoint",
            "../pacer/output/exp/pacer/valuenet_realpath_JTA+JRDB_valuenet_00025000.pth",
        )
        config["MODEL"]["valuenet_checkpoint"] = valuenet_ckpt
        if valuenet_ckpt != "":
            logger.info(f"Loading checkpoint from {valuenet_ckpt}")
            valuenet.load_state_dict(
                torch.load(valuenet_ckpt, map_location=torch.device("cpu"))
            )
        else:
