...
parser.add_argument(
    "--time_exp", type=int, default=0, metavar="N", help="timing experiment"
)
parser.add_argument(
...
parser.add_argument("--subset", type=str, default="eth", help="Name of the subset.")
parser.add_argument(
    "--model_save_dir",
    type=str,
    default="eth_ucy/saved_models",
    help="Name of the subset.",
)
parser.add_argument("--scale", type=float, default=1, metavar="N", help="dataset scale")
...
parser.add_argument(
    "--model_name", type=str, default="ckpt_best", metavar="N", help="dataset scale"
)
parser.add_argument(
    "--test_scale", type=float, default=1, metavar="N", help="dataset scale"
)
...
def main():
    ...
    if args.test:
        ...
        (
            ...
            out_ade,
            out_fde,
        ) = test(model, optimizer, 0, loader_test, backprop=False, valuenet=valuenet)
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            print(
                "Best Test Loss: %.5f \t Best ade: %.5f \t Best epoch %d"
...
def train(model, optimizer, epoch, loader, valuenet, backprop=True):
        ...
        if data is not None:
            ...
            if args.supervise_all:
                ...
                loss = torch.mean(
                    torch.min(
                        torch.mean(
                            torch.norm(mask * (loc_pred - loc_end), dim=-1), dim=3
                        ),
                        dim=2,
                    )[0]
                ...
                loss = torch.mean(
                    torch.min(
                        torch.mean(
                            torch.norm(loc_pred[:, 0:1] - loc_end[:, 0:1], dim=-1),
                            dim=3,
                        ),
                        dim=2,
                    )[0]
    ...
    if valuenet is not None:
        print(
            "%s epoch %d mse loss: %.5f value loss: %.5f"
            % (
                prefix,
                epoch,
                (res["loss"] - res["valueloss"]) / res["counter"],
                res["valueloss"] / res["counter"],
            )
        )
    else:
        print(
            "%s epoch %d avg loss: %.5f" % (prefix, epoch, res["loss"] / res["counter"])
        )
...
def test(model, optimizer, epoch, loader, backprop=True, valuenet=None):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            if data is not None:
                ...
                ades = np.mean(
                    np.linalg.norm(
                        loc_pred[:, 0:1, :, :, :] - loc_end[:, 0:1, :, :, :], axis=-1
                    ),
                    axis=3,
                )
                ...
                fdes = np.linalg.norm(
                    loc_pred[:, 0:1, :, -1, :] - loc_end[:, 0:1, :, -1, :], axis=-1
                )
                ...
                if valuenet is not None:
                    ...
                    candidates = torch.zeros(
                        (pred_trajs.shape[0], pred_trajs.shape[2], 2)
                    )
                    pred_values = torch.zeros(
                        (pred_trajs.shape[0], pred_trajs.shape[2])
                    )
                    for i in range(pred_trajs.shape[2]):
                        pred_value, value_loss = valuenet.calc_embodied_motion_loss(
                            pred_trajs[:, 0, i], None, init_vels[:, 0]
                        )
                    ...
                    for i, scene in enumerate(candidates):
                        ...
                        if (
                            filtered_candidates.shape[0] == 0
                        ):  # use the prediction with maximum value
    ...
    print(
        ...
        % (
            prefix + "test",
            epoch,
            res["minfde"] / res["counter"],
            res["minade"] / res["counter"],
        )
    )
    ...
    return (
        res["minfde"] / res["counter"],
        res["minade"] / res["counter"],
        res["meanade"] / res["counter"],
        res["meanfde"] / res["counter"],
        res["filtered_ade"],
        res["filtered_fde"],
        res["out_ade"],
        res["out_fde"],
