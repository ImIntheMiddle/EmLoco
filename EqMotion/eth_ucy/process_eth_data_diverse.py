...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_num", type=int, default=5, help="Name of the subset.")
    ...
    generator_train = data_generator(
        args.subset, past_length, future_length, scale, split="train", phase="training"
    )
    generator_test = data_generator(
        args.subset, past_length, future_length, scale, split="test", phase="testing"
    )
    ...
    os.makedirs("processed_data_diverse", exist_ok=True)
    np.save("processed_data_diverse/" + args.subset + "_data_train.npy", all_data)
    np.save("processed_data_diverse/" + args.subset + "_num_train.npy", all_valid_num)
    ...
    print(all_data.shape)
    print(all_valid_num.shape)
    np.save("processed_data_diverse/" + args.subset + "_data_test.npy", all_data)
    np.save("processed_data_diverse/" + args.subset + "_num_test.npy", all_valid_num)
