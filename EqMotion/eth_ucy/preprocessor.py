...
class preprocess(object):
    ...
    def get_valid_id(self, pre_data, fut_data):
        ...
        for idx in cur_id:
            exist_pre = [
                (False if isinstance(data, list) else (idx in data[:, 1]))
                for data in pre_data[: self.min_past_frames]
            ]
            exist_fut = [
                (False if isinstance(data, list) else (idx in data[:, 1]))
                for data in fut_data[: self.min_future_frames]
            ]
            ...
            for j in range(self.past_frames):
                ...
                if len(past_data) > 0 and identity in past_data[:, 1]:
                    found_data = (
                        ...
                        / self.past_traj_scale
                    )
                    box_3d[self.past_frames - 1 - j, :] = torch.from_numpy(
                        found_data
                    ).float()
                    mask_i[self.past_frames - 1 - j] = 1.0
                elif j > 0:
                    box_3d[self.past_frames - 1 - j, :] = box_3d[
                        self.past_frames - j, :
        ...
        for identity in valid_id:
            ...
            for j in range(self.future_frames):
                ...
                if len(fut_data) > 0 and identity in fut_data[:, 1]:
                    found_data = (
                        fut_data[fut_data[:, 1] == identity].squeeze()[
                            [self.xind, self.zind]
                        ]
                        / self.traj_scale
                    )
    ...
    def __call__(self, frame):
        ...
        assert (
            frame - self.init_frame >= 0
            and frame - self.init_frame <= self.TotalFrame() - 1
