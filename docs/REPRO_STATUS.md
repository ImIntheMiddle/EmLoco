...
| 28 | "Self-trained valuenet → used in `train_*.py --valueloss_w 1.0` → eval matches paper" | 🔴 | Retried in earnest after the user clarified the CVPR `policy_v4_realpath_JTA+JRDB_00005000.pth` was trained to **5 000 iter, not 150 000** (`tensorboard` events on plausibl confirm 5k iter ≈ 55 GPU-hours on the original dl41). Launched `policy_pretrain` (num_envs=512, max_iter=5000, GPU 4) and `valuenet_train` (num_envs=160, --load_path=plausibl 5k policy ckpt, GPU 5) in parallel with `--no_log` and a monkey-patched `humanoid.py` (`num_jobs=1` to bypass the multiprocessing env-creation hang). Both processes sat alive for 1+ hr (~5 GB RSS, GPU memory allocated but 0 % util). `/proc/<pid>/wchan` revealed `unix_stream_data_wait` — the processes are deadlocked on a Unix-socket read (PyTorch/Isaac Gym CUDA IPC), not legitimate compute. Even after the deadlock, 5k iter ≈ 55 hr and 25k iter ≈ 30+ hr per plausibl tensorboard — not feasible in a session. Pre-published `valuenet_realpath_JTA+JRDB_valuenet_00025000.pth` (on HF via #commit 787c6f8) is the supported byte-locked checkpoint for the EmLoco-loss path |
