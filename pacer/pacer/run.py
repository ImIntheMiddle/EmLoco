...
def build_alg_runner(algo_observer):
    ...
    runner.algo_factory.register_builder(
        ...
        lambda **kwargs: amp_continuous_value.AMPValueAgent(**kwargs),
    )
    runner.player_factory.register_builder(
        "amp_continuous_value",
    ...
    runner.model_builder.model_factory.register_builder(
        "continuous_amp_sept",
        lambda network, **kwargs: amp_sept_models.ModelAMPContinuousSept(network),
    )
    runner.model_builder.model_factory.register_builder(
        "continuous_amp_sept_value",
        lambda network, **kwargs: amp_sept_value_models.ModelAMPContinuousSeptValue(
            network
        ),
    )
    ...
    runner.model_builder.network_factory.register_builder(
        "amp_sept", lambda **kwargs: amp_network_sept_builder.AMPSeptBuilder()
    )
    runner.model_builder.network_factory.register_builder(
        "amp_sept_value",
        lambda **kwargs: amp_network_sept_value_builder.AMPSeptValueBuilder(),
    )
    runner.model_builder.network_factory.register_builder(
        "amp_sept_cnn",
        lambda **kwargs: amp_network_sept_cnn_builder.AMPSeptCNNBuilder(),
    )
...
def main():
    ...
    (
        ...
        flags.slow,
        flags.height_debug,
        flags.random_heading,
        flags.no_virtual_display,
        flags.render,
        flags.init_heading,
        flags.heading_inversion,
        flags.adjust_root_vel,
        flags.input_init_pose,
        flags.add_noise,
        flags.vru,
    ) = (
        args.debug,
        ...
        cfg_train["params"]["config"]["player"].get("render", False),
        args.init_heading,
        args.heading_inversion,
        args.adjust_root_vel,
        args.input_init_pose,
        args.add_noise,
        args.vru,
    )
    ...
    if args.real_path != "":
        flags.real_path = True
        if "JTA" in args.real_path:
            flags.jta_path = True
        if "JRDB" in args.real_path:
            flags.jrdb_path = True

    if args.valuenet_path != "":
        cfg_train["params"]["config"]["player"]["valuenet_path"] = args.valuenet_path

    cfg_train["params"]["config"]["player"]["use_pose"] = args.input_init_pose
    cfg_train["params"]["config"]["player"]["use_vel"] = args.input_init_vel
    cfg_train["params"]["config"]["player"]["use_vru"] = args.vru
    print("Use pose: ", cfg_train["params"]["config"]["player"]["use_pose"])
    print("Use vel: ", cfg_train["params"]["config"]["player"]["use_vel"])
