...
class HumanoidPedestrianTerrain(humanoid_traj.HumanoidTraj):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id,
        ...
        self.location_coefficient = cfg["env"].get("location_coefficient", 1)
        ...
        if (flags.render) and self.show_sensors:
        ...
        if not self._traj_gen._flags.vru:
            self.waypoint_traj = torch.zeros(self.num_envs, self._num_traj_samples, 3).to(self.device)
        else:
            self.waypoint_traj = torch.zeros(self.num_envs, 5, 3).to(self.device)
        self.init_pose = torch.zeros(self.num_envs, 24, 3).to(self.device)
        self.init_vel = torch.zeros(self.num_envs, 2).to(self.device)
    ...
    def _draw_task(self):
        ...
        if flags.show_traj:
            ...
            for i, env_ptr in enumerate(self.envs):
                ...
                cols = np.array([agent_color[i]], dtype=np.float32)
                ...
                for marker_handle in self._marker_handles[i]:
                    self.gym.set_rigid_body_color(env_ptr, marker_handle, 0,
                                gymapi.MESH_VISUAL, gymapi.Vec3(agent_color[i][0], agent_color[i][1], agent_color[i][2]))
        else:
            self._marker_pos[:] = 0
            self.gym.set_actor_root_state_tensor_indexed(
                self.sim, gymtorch.unwrap_tensor(self._root_states),
    ...
    def get_head_pose(self, env_ids=None):
        if self.smpl_humanoid:
            ...
            head_idx = 2
        head_pose = torch.cat([
            self._rigid_body_pos[:, head_idx], self._rigid_body_rot[:,head_idx]], dim=1)
        if (env_ids is None):
            return head_pose
        else:
    ...
    def _compute_flip_task_obs(self, normal_task_obs, env_ids):
        ...
        traj_samples_dim = 2 * self._num_traj_samples
        obs_acc = []
        normal_task_obs = normal_task_obs.clone()
        traj_samples = normal_task_obs[:, :traj_samples_dim].view(B, self._num_traj_samples, 2)
        traj_samples[..., 1] *= -1
        ...
        if not flags.server_mode:
            ...
            if len(env_ids) > 0:
                ...
                if not self._traj_gen._flags.vru:
                    ...
                    self.waypoint_traj[env_ids] = self._fetch_vru_waypoints(env_ids)
                self.init_pose[env_ids] = self._rigid_body_pos[env_ids]
                self.init_vel[env_ids] = root_vel[:, :2]

                if self.headless:
                    # update camera position based on the initial position
                    cam_target = gymapi.Vec3(root_pos[0, 0], root_pos[0, 1], 0)
                    cam_pos = gymapi.Vec3(root_pos[0, 0], root_pos[0, 1], 7)
    ...
    def _sample_ref_state(self, env_ids, vel_min=1, vel_range=0.5):
        ...
        if (self._state_init == HumanoidAMP.StateInit.Random
            ...
            # motion_times[0] = 2.2063
        elif (self._state_init == HumanoidAMP.StateInit.Default):
            motion_ids = torch.zeros(num_envs, dtype=torch.int32, device=self.device)
            motion_times = torch.zeros(num_envs, device=self.device)
        else:
            assert (
                False
        ...
        print(f"motion_ids: {motion_ids}, motion_times: {motion_times}")
        if self.smpl_humanoid:
            env_ids = env_ids.to(self.device)
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        if flags.fixed:
            ...
            new_root_xy[:, 0], new_root_xy[:, 1] = 50, 55
    ...
    def create_training_ground(self):
        ...
        self.gym.add_triangle_mesh(self.sim,
                                   ...
                                   tm_params)
        self.height_samples = self.terrain.heightsamples.view(self.terrain.tot_rows, self.terrain.tot_cols).to(self.device)
