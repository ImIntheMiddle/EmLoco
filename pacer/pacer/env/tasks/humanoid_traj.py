...
class HumanoidTraj(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.step_to_pred = cfg["env"]["stepToPred"]
        self.inverted = None
    ...
    def _build_traj_generator(self):
        ...
        self._traj_gen = traj_generator.TrajGenerator(num_envs, episode_dur, num_verts,
                                                      ...
                                                      self._accel_max, self._sharp_turn_prob, self._motion_lib, hybridInitProb=self._hybrid_init_prob, flags=flags)

        env_ids = torch.arange(self.num_envs, device=self.device, dtype=torch.long)
        root_pos = self._humanoid_root_states[:, 0:3]
        # root_rot = torch_utils.calc_heading(self._humanoid_root_states[:, 3:7])
        root_vel = self._humanoid_root_states[:, 7:10]
        motion_ids = self._sampled_motion_ids[env_ids]
        motion_times = self._motion_start_times[env_ids]
    ...
    def _fetch_vru_waypoints(self, env_ids=None):
        # timestep: [0.44, 0.96, 1.48, 2.00, 2.52]

        if (env_ids is None):
            env_ids = torch.arange(self.num_envs, device=self.device, dtype=torch.long)

        timestep_beg = self.progress_buf[env_ids] * self.dt # begin time
        timesteps = torch.Tensor([0.44, 0.96, 1.48, 2.00, 2.52]).to(self.device) # 5 waypoints on VRU dataset
        traj_timesteps = timestep_beg.unsqueeze(-1) + timesteps

        env_ids_tiled = torch.broadcast_to(env_ids.unsqueeze(-1), traj_timesteps.shape)

        traj_samples_flat = self._traj_gen.calc_pos(env_ids_tiled.flatten(), traj_timesteps.flatten())
        traj_samples = torch.reshape(traj_samples_flat, shape=(env_ids.shape[0], 5, traj_samples_flat.shape[-1]))
