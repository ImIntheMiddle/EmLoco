...
class Humanoid(BaseTask):
    ...
    def load_smpl_configs(self, cfg):
        ...
        if self._divide_group:
            ...
            self._group_ids = torch.tensor(np.arange(cfg["env"]["numEnvs"] / group_num_people).repeat(group_num_people).astype(int)).to(self.device)
    ...
    def pre_physics_step(self, actions):
        ...
        if (self._pd_control):
            ...
            forces = self.actions * self.motor_efforts.unsqueeze(
                0) * self.power_scale
