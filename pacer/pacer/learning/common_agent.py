...
class CommonAgent(a2c_continuous.A2CAgent):
    def __init__(self, base_name, config):
        ...
        if self._do_finetune:
            ...
            self.vnet_optimizer = torch.optim.AdamW(
                self.valuenet.parameters(), lr=1e-3, weight_decay=0.0001
            )
            self.vnet_scheduler = CosineAnnealingLR(
                self.vnet_optimizer, warmup_epochs=20, max_epochs=20000
            )
            self.vnet_criterion = torch.nn.MSELoss(
                reduction="sum"
            )  # sum because we are averaging over minibatch
            ...
            self.mini_epochs_num = (
                1  # only one epoch for finetuning because we are not training policy
            )
    ...
    def init_tensors(self):
        super().init_tensors()
        self.current_combined_rewards = (
            torch.zeros_like(self.current_rewards).flatten().to(self.ppo_device)
        )
        self.game_combined_rewards = (
            torch.zeros_like(self.current_rewards).flatten().to(self.ppo_device)
        )
        self.discount_coefs = (
            torch.ones_like(self.current_rewards).flatten().to(self.ppo_device)
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if self._do_finetune:
                        vnet_loss = (
                            self.vnet_loss / len(self.vnet_pred)
                            if len(self.vnet_pred) > 0
                            else 0
                        )
                        mean_pred = (
                            self.vnet_pred.mean().item()
                            if len(self.vnet_pred) > 0
                            else 0
                        )
                        mean_combined_rewards = (
                            self.vnet_gt.mean().item() if len(self.vnet_gt) > 0 else 0
                        )
                ...
                if self.save_freq > 0 and (epoch_num % self.save_freq == 0):
                    if not self._do_finetune:
                        ...
                        torch.save(
                            ...
                            model_output_file + "_valuenet.pth",
                        )
                    print("latest model saved")

                    if (self._save_intermediate) and (
                        epoch_num % (self.save_freq * 5) == 0
                    ):
