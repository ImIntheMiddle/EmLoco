...
class AMPValueAgent(amp_continuous.AMPAgent):
    ...
    def _load_config_params(self, config):
        ...
        self._tv_coef = config.get('tv_coef', None)
        return
    ...
    def play_steps(self):
        ...
        # gt_accum_reward = torch.zeros(self.num_actors, device=self.device)
        ...
        for n in range(self.horizon_length):
            with torch.no_grad():
                ...
                # gt_accum_reward += infos['reward_raw'].sum(dim = 1)
                ...
                combined_rewards = shaped_rewards.clone().detach().flatten() + amp_rewards
                self.current_combined_rewards += (combined_rewards * self.discount_coefs)
                ...
                self.done_early = torch.logical_and((self.current_lengths <= self.step_to_pred), self.dones)
                self.over_pred_steps = torch.logical_and((self.current_lengths == self.step_to_pred), not_dones)
                # self.game_combined_rewards += self.current_combined_rewards * self.dones.float()
                ...
                self.discount_coefs[done_indices] = 1.0
            ...
            if self._do_finetune and (len(valid_rewards_idx[0]) > 0):
                ...
                valuenet_pred = self.valuenet(self.waypoint_traj[:, :13, :], self.init_pose, self.init_vel).squeeze()

                game_combined_rewards = self.game_combined_rewards.clone().detach()
                # self.max_cum_rewards = max(self.max_cum_rewards, game_combined_rewards.max().item())
                # self.min_cum_rewards = min(self.min_cum_rewards, game_combined_rewards.min().item())

                self.normalized_rewards = (game_combined_rewards[valid_rewards_idx] - self.min_cum_rewards) / (self.max_cum_rewards - self.min_cum_rewards)

                vnet_loss = self.vnet_criterion(valuenet_pred[valid_rewards_idx], self.normalized_rewards)
                vnet_loss.backward()
        ...
        with torch.no_grad():
            ...
            for k, v in amp_rewards.items():
                ...
                # gt_accum_reward += v.sum(dim=0).squeeze()

            # batch_dict['gt_reward'] = gt_accum_reward/self.horizon_length
    ...
    def train_epoch(self):
        ...
        # train_info['vnet_loss'] = vnet_loss
    ...
    def calc_gradients(self, input_dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            ...
            disc_info['disc_loss'] *= self._disc_coef
            disc_loss = disc_info['disc_loss']

            loss = self._actor_coef * a_loss + self.critic_coef * c_loss - self.entropy_coef * entropy + self.bounds_loss_coef * b_loss \
                 + disc_loss + self._tv_coef * tv_loss

            if self.motion_sym_loss:
                self.sym_loss_coef = self.vec_env.env.task.cfg['env'].get("sym_loss_coef", 1)
                s_info = self._sym_loss(flip_obs_batch, orig_obs_batch)
                s_loss = s_info['sym_loss'] * self.sym_loss_coef
                s_loss = torch.mean(s_loss)
                loss += s_loss

            a_clip_frac = torch.mean(a_info['actor_clipped'].float())

            a_info['actor_loss'] = a_loss * self._actor_coef
            a_info['actor_clip_frac'] = a_clip_frac
            c_info['critic_loss'] = c_loss * self.critic_coef
            c_info['tv_loss'] = tv_loss * self._tv_coef
